...
export default function PartsInventoryPage() {
  ...
  const loadData = async () => {
    try {
      ...
      setLowStockItems(lowStockRes.filter((ls: any) => partIds.has(ls.part_id)));
    } catch (err) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manufactured parts and assemblies</p>
        </div>
        <button onClick={() => setShowReceiveModal(true)} className="btn-primary flex items-center">
          <ArrowDownTrayIcon className="h-5 w-5 mr-2" /> Receive Parts
        </button>
      </div>
      ...
      <div className="card">
        {activeTab === 'summary' && (
          <div className="overflow-x-auto">
            ...
            {summary.length === 0 && <p className="text-center text-gray-500 py-8">No parts inventory on hand</p>}
          </div>
        )}
        ...
        {activeTab === 'details' && (
          <div className="overflow-x-auto">
            ...
            {inventory.length === 0 && <p className="text-center text-gray-500 py-8">No parts inventory on hand</p>}
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-lg w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Receive Parts</h3>
            ...
            <form onSubmit={handleReceive} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={receiveForm.serial_number} onChange={(e) => setReceiveForm({...receiveForm, serial_number: e.target.value})} className="input" />
                </div>
              </div>
              <div>
      ...
      {showTransferModal && selectedItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold">Transfer Part</h3>
