...
import { useToast, DataTable, DataTableColumn, MobileDataCard } from '../components/ui';
...
const StatusPill = ({ status }: { status: string }) => (
  <span className={`inline-flex px-2 py-0.5 text-xs font-medium rounded-full ${statusBadge(status)}`}>{status}</span>
);

const ScorePill = ({ score }: { score: number }) => (
  <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${scoreBadge(score)}`}>{score}</span>
);
...
const SupplierScorecards = () => {
  ...
  const handleCreateScorecard = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (actionBusy || !scorecardForm.vendor_id) return;
      setActionBusy(true);
      setActionError('');
      try {
        ...
        setActionError(
          typeof e.response?.data?.detail === 'string'
            ...
            : 'Unable to save supplier record. Check your entries and retry.'
        );
    ...
    [scorecardForm, fetchData, showToast, actionBusy]
  );

  const handleCreateAudit = useCallback(
    async (e: React.FormEvent) => {
      e.preventDefault();
      if (actionBusy || !auditForm.vendor_id) return;
      setActionBusy(true);
      setActionError('');
      try {
    ...
    [auditForm, fetchData, showToast, actionBusy]
  );
  ...
  const statCards = useMemo(
    () => [
      ...
      },
      {
        ...
        valueColor: stats.at_risk_count > 0 ? 'text-fd-red' : undefined,
      },
      {
        label: 'Audits Due Soon',
        value: stats.audits_due_soon,
        icon: CalendarDaysIcon,
        iconBg: 'bg-fd-amber/15',
        iconColor: 'text-fd-amber',
        valueColor: stats.audits_due_soon > 0 ? 'text-fd-amber' : undefined,
      },
    ],
    [stats]
  );
  ...
  const scorecardMobileCard = useCallback(
    (sc: Scorecard) => (
      <MobileDataCard
  ...
  const aslMobileCard = useCallback(
    (s: ApprovedSupplier) => (
      <MobileDataCard
        title={s.vendor_name}
  ...
  const auditMobileCard = useCallback(
    (a: Audit) => (
      <MobileDataCard
        title={a.vendor_name}
  ...
  return (
    <div className="p-4 space-y-4">
      <Modal open={!!auditDetail} onClose={() => setAuditDetail(null)} size="lg">
        <h2 className="text-lg font-semibold">Supplier audit · {auditDetail?.vendor_name}</h2>
        <p>
          {auditDetail?.audit_date} · {auditDetail?.audit_type} · Score {auditDetail?.score}
        </p>
        <h3 className="font-semibold mt-4">Full findings</h3>
        <p className="whitespace-pre-wrap break-words">{auditDetail?.findings || 'No findings recorded'}</p>
      </Modal>
          ...
          <DataTable<Scorecard>
            ...
            empty={{
              ...
              description: search
                ...
                : 'Rated suppliers will appear here once you create a scorecard.',
      ...
      {activeTab === 'Audits' && (
        <DataTable<Audit>
          ...
          data={filteredAudits}
          onRowClick={audit => setAuditDetail(audit)}
          rowKey={a => a.id}
      ...
      >
        ...
        <form onSubmit={handleCreateScorecard} className="p-6 space-y-4">
          ...
          {actionBusy && <p role="status">Saving…</p>}
          <FormField label="Vendor" required>
            {field => (
              <EntityPicker
                ...
                onChange={value => setScorecardForm(f => ({ ...f, vendor_id: value }))}
              />
            )}
          </FormField>
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-slate-300 bg-fd-panel border border-slate-600 rounded-lg hover:bg-slate-800"
            >
              Cancel
            </button>
            <button
          ...
          <FormField label="Vendor" required>
            {field => (
              <EntityPicker
                ...
                onChange={value => setAuditForm(f => ({ ...f, vendor_id: value }))}
              />
            )}
          </FormField>
          <div className="grid grid-cols-2 gap-4">
