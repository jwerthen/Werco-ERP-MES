    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            <h1 className="text-2xl font-bold text-gray-900 truncate">{part.part_number}</h1>
            <span className={`inline-flex px-2.5 py-1 rounded-md text-xs font-semibold ${partTypeColors[part.part_type] || 'bg-gray-100 text-gray-800'}`}>
              {part.part_type.replace('_', ' ')}
            </span>
            <StatusBadge status={part.status} />
            {part.is_critical && (
              <span className="inline-flex px-2 py-0.5 rounded bg-red-100 text-red-800 text-xs font-medium">
                Critical
              </span>
            )}
          </div>
          <p className="text-gray-600 mt-1">{part.name}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide">Standard Cost</div>
          <div className="text-lg font-semibold mt-0.5">${Number(part.standard_cost || 0).toFixed(2)}</div>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide">BOM Status</div>
          <div className="mt-0.5">
            {bom ? <StatusBadge status={bom.status} /> : <span className="text-sm text-gray-400">None</span>}
          </div>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide">Routing Status</div>
          <div className="mt-0.5">
            {routing ? <StatusBadge status={routing.status} /> : <span className="text-sm text-gray-400">None</span>}
          </div>
        </div>
        <div className="bg-white border border-gray-200 rounded-lg p-3">
          <div className="text-xs text-gray-500 uppercase tracking-wide">Inspection</div>
