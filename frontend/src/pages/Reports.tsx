...
export default function Reports() {
  ...
  const activeTab: TabType = TAB_IDS.includes(tabParam) ? tabParam : 'dashboard';
  ...
  const setActiveTab = (tab: TabType) => updateQuery('tab', tab);
  const setPeriod = (value: number) => updateQuery('period', String(value));
  ...
  const loadData = useCallback(async () => {
    ...
    const [prodRes, qualRes, invRes, vendRes, utilRes, dailyRes, costRes, timeRes] = await Promise.allSettled([
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        tabs={[
          { id: 'dashboard', label: 'Dashboard', panelId: 'reports-dashboard' },
          { id: 'ship-otd', label: 'Ship OTD', panelId: 'reports-ship-otd' },
          { id: 'costing', label: 'Work Order Costing', panelId: 'reports-costing' },
          { id: 'timesheets', label: 'Employee Time', panelId: 'reports-timesheets' },
        ]}
        activeTab={activeTab}
      ...
      <div
        id={`reports-${activeTab}`}
        role="tabpanel"
        aria-labelledby={`reports-${activeTab}-tab`}
        className="space-y-6"
      >
        <p className="text-xs text-slate-400">
          The selected period applies to production, quality, utilization and costing. Daily Output uses 14 days, Vendor
          Performance 90 days, and Employee Time 7 days.
        </p>
        {activeTab === 'dashboard' && (
          <>
          ...
          compact retry affordance. */}
            ...
            <MiniStatStrip className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7 gap-2">
              <MiniStat
                ...
                subtitle={
                  errors.production
                    ? 'Unavailable'
                    : `${production?.on_time_delivery_count ?? 0} of ${production?.total_completed ?? 0}`
                }
              />
              <MiniStat
                icon={ClockIcon}
              ...
              <MiniStat
                ...
                value={
                  errors.inventory
                    ? '—'
                    : `$${(inventory?.total_value || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                }
              ...
              <MiniStat
                ...
                iconColor="text-fd-blue"
                label="Qty Received"
                value={errors.quality ? '—' : (quality?.receiving_total_qty ?? 0).toLocaleString()}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-12 gap-4 items-start">
                    ...
                    {utilization.slice(0, 8).map(wc => (
                      <div key={wc.work_center_id} className="min-w-0">
                        <div className="flex justify-between text-sm mb-1 gap-2">
                          ...
                          >
                            {wc.utilization_pct}%{wc.utilization_pct > 100 ? ' · above capacity' : ''}
                          </span>
                        </div>
              ...
              <CockpitPanel title="Top Vendor Performance" subtitle="Last 90 days" className="xl:col-span-7">
                {errors.vendors ? (
            ...
            panel carries only the per-category breakdowns. */}
              <CockpitPanel title="Quality & Work Orders" subtitle="By source / status" className="xl:col-span-5">
                {errors.quality || errors.production ? (
                  ...
                  <>
                    {quality && Object.keys(quality.ncr_by_source).length > 0 && (
                      <div className="mb-3">
                        <h3 className="text-[10px] font-medium uppercase tracking-wide text-slate-500 mb-2">
                          NCRs by Source
                        </h3>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {production &&
                          Object.entries(production.work_orders_by_status).map(([status, count]) => (
                        ...
                        {costing.map(wo => (
                          <tr key={wo.work_order_id} className="border-b border-fd-line hover:bg-fd-sunken">
                            <td className="py-3 px-4">
                              <div className="font-medium text-fd-link">{wo.work_order_number}</div>
                    ...
                    <div className="flex justify-between items-center bg-fd-sunken px-4 py-2 rounded-t-sm">
                      ...
                      <div className="flex flex-wrap items-center justify-end gap-3 text-sm">
                        <span className="text-emerald-400 font-bold tabular-nums">
                          {emp.completed_operations || 0} ops
                        </span>
                        <span className="text-slate-300 font-semibold tabular-nums">
                          {emp.quantity_produced || 0} qty
                        </span>
                        <span className="text-fd-link font-bold tabular-nums">{emp.total_hours} hrs</span>
                      </div>
                    </div>
                    <div className="border border-t-0 border-fd-line rounded-b-sm overflow-x-auto">
                      <div className="flex flex-wrap justify-between gap-3 px-4 py-2 text-sm">
                        <span>
                          {expandedEmployees.has(emp.user_id) ? emp.entries.length : Math.min(10, emp.entries.length)}{' '}
                          of {emp.entries.length} entries · total includes all entries
                        </span>
                        {emp.entries.length > 10 && (
                          <button
                            className="text-fd-blue underline"
                            onClick={() =>
                              setExpandedEmployees(current => {
                                const next = new Set(current);
                                if (next.has(emp.user_id)) next.delete(emp.user_id);
                          ...
                          >
                            {expandedEmployees.has(emp.user_id)
                              ? 'Show first 10'
                              : `Show all ${emp.entries.length} entries for ${emp.employee_name}`}
                          </button>
                        )}
                      </div>
                      <table className="min-w-full text-sm">
                        ...
                        <tbody>
                          {(expandedEmployees.has(emp.user_id) ? emp.entries : emp.entries.slice(0, 10)).map(
                            (entry, idx) => (
                              <tr key={idx} className="border-t">
                                ...
                                <td className="py-2 px-4">
                                  {entry.completed_at
                                    ? formatCentralDate(entry.completed_at, { month: 'short', day: 'numeric' })
                                    : '-'}
                                </td>
