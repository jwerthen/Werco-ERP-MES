...
### Sales & Quoting
- **Fabrication Quoting** — a reviewed assembly/BOM estimator at `/fabrication-quotes`, with DXF/STEP/PDF evidence, source hashes, Decimal cost calculations, laser/forming/welding/manual operations, internal stock nesting, hardware supplier-offer CSV import and reusable process profiles.
- **Controlled release** — immutable approved revisions, explicit review of unknowns and assumptions, quantity scenarios, manual actual-work capture and an audited handoff to an ERP customer quote.
- **Customer quote management** — existing customer quote documents, sending/export and production conversion remain in Quotes. New manufacturing estimates begin in Fabrication Quoting.
- The old AI RFQ, Quote Calculator, Estimate Workbench, Shop Data and standalone Material Nesting quoting applications are retired. Their old UI links redirect to the replacement; historical database records and migrations are retained without repricing. Production laser-nest import and dispatch remain available on work orders.
- Shop rates and supplier offers require estimator review. The system does not claim a cost-error percentage before Werco qualification trials. See [Fabrication Quoting](docs/FABRICATION_QUOTING.md).
...
| [docs/MATERIAL_NESTING.md](docs/MATERIAL_NESTING.md) | Sheet material estimator: DXF import, size comparisons, limits, and local estimate files |
