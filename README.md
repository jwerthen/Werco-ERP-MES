...
- **Material Nesting** — compare actual-contour sheet layouts for separate material/thickness groups at `/nest`, inside the authenticated ERP. Optional read-only ERP catalog pricing requires estimator review; local saves and draft review exports retain source provenance without creating quotes or inventory transactions. Estimates use inches. See [docs/MATERIAL_NESTING.md](docs/MATERIAL_NESTING.md).
