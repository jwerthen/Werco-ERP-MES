...
- **TV wallboard** — read-only, full-screen `/wallboard` "Andon Wall" for shop TVs: computed shop-state headline, alarm-first work-center grid with live jobs, a SHIP/LATE/BLOCKED·DOWN/QUALITY exception rail, and a today + 30-day KPI band; 30s refresh, per-department filter; authenticated by scoped, revocable display tokens that can reach no other endpoint (see [docs/WALLBOARD.md](docs/WALLBOARD.md)).
