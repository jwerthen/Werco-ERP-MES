...
- **Operation dependency DAG** — Carbon's `jobOperationDependency` table with DB-enforced cycle prevention and trigger-driven Waiting→Ready propagation models "assembly waits for 3 parallel subassemblies." Werco's `has_incomplete_predecessors()` models a **partial** order only — strict across work centers, unordered within one (with an ON_HOLD override), derived from `sequence` + `work_center_id`. There is no dependency table, no cycle prevention, and no cross-work-order dependency, so the gap stands.
