...
- Audits 52136–52155 changed the other 20 operations from ready to complete with
  `source=reconcile_on_read` and empty `time_entry_ids`. Their planned quantities,
  operator, and timestamps were copied from operation 882 without production.
...
The same signature appears on replaced work order 105. The user confirmed that
106 replaced it; this repair does not reopen or otherwise change 105.
...
evidence; any new activity aborts. It restores the 20 false completions to ready
with zero production and no borrowed stamps, preserves operation 882 and entry
496, and returns 106 to in-progress with zero finished assemblies.
