...
- **Fill `unit_of_measure` for anything you don't actually stock in each** — sheet, plate, bar, wire, coating, adhesive. It still defaults to `each` here (unlike a BOM line, which now inherits from the part), and **this column is the value BOM lines inherit from in step 7**. Getting it right once here is how a sheet-metal BOM comes out saying `sheets` without anyone typing it. Getting it wrong is silently consistent — the part says `each`, the lines inherit `each`, nothing flags it — until you correct the part later and every line that quietly agreed starts blocking automatic backflush. It is far cheaper to be right in this spreadsheet than to remediate afterwards.
...
- **Leave `unit_of_measure` blank unless the line genuinely differs from how you stock the component.** A blank cell now inherits the **component part's own** unit of measure (owner decision, 2026-07-27); it no longer becomes a literal `each`. A filled cell always wins, and the wizard normalises the common spellings (`ea` → `each`, `lbs` → `pounds`). So on a sheet-metal BOM, a component that **already exists** stocked in **sheets** imports as `sheets` when you leave the cell alone — and typing `each` out of habit is now the thing that creates a wrong record. Blank is the safe answer.
  - **One trap, for components the wizard CREATES during commit:** that same cell sets the **new part's own** stocking unit too, so a blank cell makes both the part and the line `each` — consistent, and therefore silent, even if you actually stock the thing in sheets. If you later correct the part on the Materials page, the line that agreed with it stops agreeing and starts blocking. This is the argument for doing **steps 5 and 6 first** so components already exist with their real units, and for filling the cell when you knowingly let the wizard create a component you don't stock in each.
- **This column decides whether the assembly can later be armed for automatic backflush.** A stored unit that contradicts the component part's is treated as a *stated* claim and **blocks** that part from opting into automatic component consumption — nothing in the system converts units, so `each` against a part stocked in `sheets` would issue the wrong quantity. Nothing rewrites lines imported before 2026-07-27, so if you are migrating onto an existing BOM set, work the mismatch list afterwards. **You do not need the API for this** — open **Engineering → BOM Unit Mismatches** (`/bom/uom-mismatches`, Admin/Manager/Supervisor; the same data as `GET /api/v1/bom/uom-mismatches`, a pure read that changes nothing). The sequence is: **run it unfiltered → click through to the BOM and correct each line → re-check the part** (`GET /parts/{id}/backflush-readiness`, or the backflush card on the part page — every row links to it) **→ then arm the part**. Two things to read carefully on that screen: if the amber *"Scan ceiling reached"* banner is up, the count is a **floor, not a total** (narrow the filters and re-run), and the **Line effect** column answers the *line*, not your part — a line inside a sub-assembly can read *Would be issued* and still block nothing, so the part's own readiness check is the answer that counts. See [MATERIAL_CONSUMPTION_PLAN.md](MATERIAL_CONSUMPTION_PLAN.md) → *Exposing the flag* and [API.md](API.md) → *BOM line unit of measure*. This affects **backflush arming only** — a BOM with mixed-up units still imports, releases, explodes, and drives MRP exactly as before.
- **Bloated/messy Excel files:** the wizard's scan is bounded like the direct imports (see **Blank rows** and **Limits** under [File basics](#file-basics-read-once)), with two differences: the wizard reads **all sheets** (not just the first), and a run of more than 1,000 consecutive blank rows ends **that sheet's** scan quietly — later sheets are still read, and rows past such a gap are dropped rather than refused, so **check the preview to confirm every expected line item parsed** before committing. A workbook whose used range scans past 100,000 raw rows is refused, as is one the wizard can't read at all ("Could not read the Excel file…") — the fix is the same: delete trailing blank rows/columns or re-save the data, then try again.
...
- **Endpoints:** `POST /api/v1/routing/import/preview` (the dry-run) then `POST /api/v1/routing/import/commit` (both multipart, form field `file`, CSV or XLSX; plus an optional `assignments` JSON field the wizard fills in — see *Assigning work centers in the wizard* below). **Template:** `routings`.
- **Required columns (per row):** `part_number` (must exist — steps 5/6, and must be a `manufactured` or `assembly` part), `sequence` (whole number, **unique within a part**), `operation_name`.
- **Optional columns:** `work_center_code` (**optional** — leave it blank to assign the work center in the wizard after upload; if filled, it must match an **active** work center from step 1, or that row errors as a likely typo), `routing_revision` (default `A`), `routing_description` (the first non-empty value across a part's rows is used), `setup_hours` / `run_hours_per_unit` (numeric, default 0), `description` (stored as the operation's work instructions), `is_inspection_point`, `is_outside_operation` (`Y/N`/`true/false`, default false).
- **Assigning work centers in the wizard:** because `work_center_code` is optional, the preview returns **one row per operation** with a work-center dropdown, flagging every operation that still `needs_work_center` (a `operations_needing_work_center` count is shown too). Pick a work center for each unassigned operation, then **Commit** — the wizard sends your picks back in the `assignments` field. Resolution precedence per operation is: **your wizard assignment wins** — an explicit pick **overrides** any file `work_center_code` on that row — otherwise the file `work_center_code` is used. The file code is just a **default that pre-fills the dropdown** (a preview with no picks resolves it so the dropdowns come up populated from the file); an explicit assignment always overrides it. You can leave operations unassigned in **preview** (it writes nothing), but **commit requires every operation to have a work center** (see below).
- **How it groups:** rows are grouped by `part_number` — **one part = one draft routing** plus all its operations, in first-seen file order. Every routing is created in **`draft`** status; you still **release** each one afterward (step 11 *refuses* any part without a released routing).
- **Parts are never created.** The part must already exist; a `part_number` that isn't a manufactured/assembly part (or is soft-deleted) fails the whole routing.
- **Same-revision conflict rule — read this:** the importer **never mutates an existing routing.** If the part already has a routing at the **same revision** you're importing, that routing fails with "part 'X' already has a routing at revision 'A' — choose a new revision". Any *other* revision is fine — a new **draft** revision is created alongside the existing ones, which are left untouched (compliance: prefer new revisions over editing shipped data).
- **Whole-or-not-at-all per part:** a routing imports as a unit — if any one of a part's rows fails validation (bad sequence, a duplicate sequence within the part, an unknown/inactive work center code, or — at commit — an operation still missing a work center), the sibling rows of that part are skipped too (reported as "skipped: row N in the same routing failed validation"). Commit is **per-routing**, so one bad part never poisons the rest. **No routing is ever created with an unassigned operation** — if any operation still lacks a work center at commit, that whole routing errors and is not created.
- **What it creates:** one **draft** routing per part with its operations; each created routing writes one tamper-evident audit-log CREATE (`resource_type` `routing`, source `import`).
- **Common errors:** "part 'X' not found"; "part 'X' is not a manufactured or assembly part"; "duplicate sequence N for part 'X'"; "work center 'X' not found or inactive" (a non-blank `work_center_code` that doesn't resolve); "assigned work center id N not found or inactive" (a bad wizard assignment); "part 'X' has operations without a work center (row(s) …) — assign a work center before committing"; "part 'X' already has a routing at revision 'A' — choose a new revision".

This is usually the longest step of the migration — budget real time for it, and prioritize the parts that have open work orders (step 11 *refuses* any part without a released routing). If the importer doesn't fit a part (one-off complex routing), you can still build it on the **Routing** page manually, by **copying** a similar part's routing, or with the AI **generate-from-drawing** assist.
...
| "File must include a header row" | The first non-empty row didn't contain headers (the variant "…scanning stops after 1,000 consecutive blank rows" means the file starts with a huge blank block before any headers) | Put the column names in row 1 |
...
| "vendor 'X' was deleted — restore it (`POST /api/v1/purchasing/vendors/{id}/restore`) and re-import…" | The vendor was **deleted** (records soft-delete, they don't vanish, and a deleted vendor is no longer selectable anywhere) | Follow the message: it carries the vendor's real id, so an admin can `POST` that URL directly. **There is no screen for this yet** — if nobody on site can issue an authenticated API call, this needs your ERP admin, so raise it rather than working around it. Do **not** re-create the vendor under the same code: a deleted vendor still owns its code, so the create is refused as a duplicate, and the spreadsheet's `vendor_code` column is the join key so a different code is not an option either |
