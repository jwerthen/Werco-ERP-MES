...
- **Endpoints:** `POST /api/v1/routing/import/preview` (the dry-run) then `POST /api/v1/routing/import/commit` (both multipart, form field `file`, CSV or XLSX). **Template:** `routings`.
- **Required columns (per row):** `part_number` (must exist — steps 5/6, and must be a `manufactured` or `assembly` part), `sequence` (whole number, **unique within a part**), `operation_name`, `work_center_code` (must match an **active** work center — step 1).
- **Optional columns:** `routing_revision` (default `A`), `routing_description` (the first non-empty value across a part's rows is used), `setup_hours` / `run_hours_per_unit` (numeric, default 0), `description` (stored as the operation's work instructions), `is_inspection_point`, `is_outside_operation` (`Y/N`/`true/false`, default false).
...
- **Common errors:** "part 'X' not found"; "part 'X' is not a manufactured or assembly part"; "duplicate sequence N for part 'X'"; "work center 'X' not found or inactive"; "part 'X' already has a routing at revision 'A' — choose a new revision".
