...
- **This column decides whether the assembly can later be armed for automatic backflush.** A stored unit that contradicts the component part's is treated as a *stated* claim and **blocks** that part from opting into automatic component consumption — nothing in the system converts units, so `each` against a part stocked in `sheets` would issue the wrong quantity. Nothing rewrites lines imported before 2026-07-27, so if you are migrating onto an existing BOM set, run `GET /api/v1/bom/uom-mismatches` (Admin/Manager/Supervisor, a pure read) afterwards and correct what it lists. See [MATERIAL_CONSUMPTION_PLAN.md](MATERIAL_CONSUMPTION_PLAN.md) → *Exposing the flag* and [API.md](API.md) → *BOM line unit of measure*. This affects **backflush arming only** — a BOM with mixed-up units still imports, releases, explodes, and drives MRP exactly as before.
