...
- [x] Production-event coverage (AU-3.3.1 audited events) extended to the work-order
  ...
  **`COMPLETED_WITH_QUALITY_EXCEPTION`** (distinct from a plain completion, so a bypass is greppable in
  ...
  re-audit): its `qty_consumed` advance from 0 to `qty_planned`, written when the one-shot backflush
  drains it, is a state change on a tenant table *and* the exact field the untie guard keys on (409
  once anything is consumed), so it writes its own `work_order_material_allocation` UPDATE row
  (`extra_data.reference_type = "work_order"`). Two further tamper-evident actions were added at the
  ...
  predicate and write the same row.
  ...
  **AS9100D 8.5.2 (identification & traceability):** because the finished-goods receipt assigns and
  ...
  tenant-scoped. Genealogy reads **both** work-order ledger reference types — `work_order` (the
  finished-goods receipt and the one-shot backflush) and `work_order_operation` (per-run tied-material
  consumption, whose `reference_id` is resolved back to its work order so both collapse into the same
  per-work-order genealogy lines). That extension shipped **in the same PR as the new reference
  type**: there is deliberately no release in which consumption exists but the as-built record cannot
  see it. Tied consumption posts as `ISSUE` **even for scrapped runs** (the good/scrap split is
  ...
  truthfully carry `work_order` only, and pre-feature rows keep a NULL `allocation_id`.
  **Non-duplication of regulated inventory/traceability records — which mechanism guarantees what.**
  ...
  regulated inventory row. Two different guarantees apply, and they are not interchangeable:

  * **Work-order-scoped movements** — the finished-goods `RECEIVE` and the one-shot backflush /
    work-order-scoped-tie `ISSUE`, both `reference_type='work_order'` — are guarded **at the database
    ...
    by the material-consumption work.
