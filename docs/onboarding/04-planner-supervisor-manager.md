...
A laser-cutting job run straight from an Ermaksan nest package is the one kind of work order that doesn't need a part. On **Work Orders**, click **Import Nest Package** (next to **New Work Order**), upload the zipped nest package or a nest-report PDF — single or multi-page; a multi-page report is split into one row per nest, with the page range shown on each row — or point at a server folder, review the rows the system read from each sheet — the values are editable, and low-confidence fields are highlighted for you to double-check — pick the laser and an optional **due date** in the dispatch strip (the Ermaksan fiber laser is pre-selected; a row's **Work Center** column can send that nest to a different laser), and import. That creates a **released laser cutting work order** on its own: its quantity is the total planned **sheet runs** (not pieces), and the list shows **Nest package** where the part number would be. The whole package lands on the laser's floor queue at once — every nest is startable immediately, and operators can run them in **any order**. A routed job now pools the same way *within* a work center (all the steps sitting on one machine go ready together and run in any order), so the two differences that remain are laser-specific: a nest package pools **across** lasers when its nests are spread over more than one machine, and **holding one nest does not stop the others**. On a routed job, holding any one step at a machine stops every step of that job at that machine until the hold is cleared. To re-import the package, add a nest by hand, change the due date, or move a nest to another laser later, open that work order and use its **Laser Nest Package** card. (You'll see the button if you're an admin, manager, or supervisor.)

**The review step suggests the sheet for you.** For each nest row, the system looks up the sheet stock that matches what it read off the sheet — thickness, grade, and size — and fills the **Sheet** picker in for you when exactly one part in your material list fits. When it isn't sure it fills in nothing and offers a short list instead, with a one-line reason for each ("`0.250-60X120-A36` and `0.250-60X120-CS` both fit this nest's spec. Pick the one this job runs."), and when it can't tell at all — an unreadable thickness, or a nest report that never named a grade — the row stays blank for you to pick. Rows also show what's on hand and warn you when the package as a whole needs more sheets than you have, so a shortage shows up here rather than a month later when the count disagrees.

Treat every one of these as a **suggestion you're confirming, not an answer**. Nothing is tied until you import, and the tie is what pulls sheets out of inventory when the nest is finished — into the job's as-built record, which doesn't unwind on its own. Read the row, fix it if it's wrong, then import. A blank sheet on a row is fine too: that nest simply runs untied and its material won't be deducted. Adding one nest by hand (**Add Nest** on the Laser Nest Package card) doesn't suggest anything — with one nest in front of you, you're already looking at the sheet you're describing.
...
On the review screen, unmatched lines offer a **Search parts** typeahead: start typing a part number or a name (for example "raw material") and similar existing parts appear as you type. Clicking into the extracted **Part Number** or **Description** searches from that text too. Pick a match, or **Create as new part**.
