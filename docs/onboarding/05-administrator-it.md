...
- **Session limits.** Sign-ins refresh silently in the background. The browser signs a user out after **15 minutes of inactivity** by default. On the phone-size Shop Floor screen only, an explicit per-user, per-company **This is my personal phone** preference allows **30 minutes**. It does not apply on desktops, dedicated `/kiosk` screens, or crew-station routes; operators must keep it off on shared devices. The one-minute warning accepts a tap/key to continue, and waking a suspended phone checks the original deadline. A session that goes unused for longer than the absolute session limit (**7 days**, `SESSION_ABSOLUTE_TIMEOUT_HOURS`) can no longer be refreshed and requires a fresh sign-in. Note the absolute limit is **re-armed on every background refresh**, so it bounds how long a session may sit *idle* — it is not a fixed ceiling on total session age, and someone using the app regularly is not forced onto a re-login schedule.
...
> Heads up: **User-management actions on the Users screen — creating, editing, activating/deactivating, role changes, and password resets — are now written to the tamper-evident audit log**, each with the acting admin, the affected account, before/after values (the password itself is never recorded), and the time. You can point an auditor straight to those entries. The Work Centers gap has narrowed: **editing a work center (the pencil) is now written to the audit log** with before/after values, and so is deactivating one. Still not recorded: **creating** a work center on the screen and the row's **status dropdown** (Available / In Use / Maintenance / Offline). (Work centers loaded through **Import CSV** were already recorded.) Also new: a work center that still has live jobs on it **can't be deactivated** — the system refuses and tells you to move that work to another machine (Dispatch Board → Move to machine) or complete it first.
