...
- `services/` — business logic. Multi-step / state-changing operations belong here, not in routers.
  ...
  - **Completion inventory effects** live in `completion_inventory_service.py` (FG receipt + BOM backflush) and `material_consumption_service.py` (tied-material consumption), both driven from the single `apply_completion_inventory_effects` seam — add **work-order**-completion side-effects there, not at the call sites. Its **operation**-level twin is `apply_operation_completion_inventory_effects` (same module), called right after `finalize_operation_completion` on all four operation-completion handlers; operation-scoped tied-material consumption fires there, while the FG receipt and the BOM backflush deliberately stay work-order-scoped. See invariant 6.
