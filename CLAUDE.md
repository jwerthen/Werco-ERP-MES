...
npm run lint           # eslint src --ext .ts,.tsx  (lint:fix to autofix)
                       ...
                       # regressions FAIL CI). CI lints with --max-warnings=0 (ci-cd.yml), so there
                       # is no non-blocking warn tier — known label-association/clickable-div debt is
                       # `off` (tracked follow-up: burn down via FormField); no-autofocus is off.
...
- List/table views use the shared `<DataTable>` primitive (`components/ui/DataTable.tsx`, barrel-exported), not hand-rolled `<table>` markup — it standardizes client-side sort, client- *or* server-side pagination, row click-through, row selection + bulk actions, CSV export, and responsive mobile cards, and routes its loading / error / empty states through the `<Skeleton>` / `<ErrorState>` / `<EmptyState>` primitives above. Sort/paginate are pure (never mutate the `data` prop). Pass `serverPagination` (page/pageSize/hasNext/onPageChange) for offset-paged endpoints where the full set isn't client-side; otherwise pass `pageSize` for client paging. WorkOrders/Inventory/Purchasing (client sort+paginate+CSV) and AuditLog (server pagination) are the reference usages.
