"""Coverage for 086_part_active_before_delete (the restore-preserves-is_active sidecar).

086 is the TWIN of 082, applied to ``parts`` instead of ``vendors``: one purely additive
column over an existing table,

- ``is_active_before_delete``  Boolean, NULLABLE, no default

remembering the ``is_active`` value a part had at the moment it was soft-deleted, so
``POST /parts/{id}/restore`` puts THAT value back instead of unconditionally reactivating
the SKU. Read ``test_migration_082_vendor_active_before_delete.py`` alongside this file --
the reasoning is deliberately identical and the two must not drift.

WHY THE HAZARD IS NEW (this is the part that is not just "same as 082")
----------------------------------------------------------------------
``restore_part`` hard-coded ``is_active = True`` / ``status = "active"``, and that was
harmless for as long as "inactive but not deleted" was essentially UNREACHABLE for a part:
``PartUpdate`` carries neither column, and the only writer of them was ``delete_part``,
which also sets ``is_deleted``. The Combine/Merge SKUs feature deliberately creates that
state AT SCALE -- ``POST /parts/{id}/deactivate`` and the combine's ``deactivate_source``
flag retire the folded-away SKU as a routine, reasoned, audited step -- so a part somebody
had deliberately retired, then deleted, then restored came back ACTIVE and selectable again
in every picker, with no audit row saying anybody decided to re-activate it. Invariant 3:
**a restore returns the RECORD, not the permission**, and it names ``is_active`` as a delete
MASK rather than a filter.

The endpoint behavior is covered in ``tests/test_part_activation.py`` (the "P1" section) and
``tests/api/test_parts_audit_persistence.py``; this file covers the MIGRATION.

Two complementary layers, mirroring 081/082:

1. Script wiring + source/model lock-step (unit) -- single alembic head, the 085 -> 086
...
-----------------------------------------------
The column is NULLABLE with NO server default, and that is not laziness. NULL has to stay
REACHABLE because it is a MEANINGFUL value: it means "we never recorded one", which is the
true state of every part soft-deleted before 086 shipped -- and, permanently, of every part
deleted through ``DELETE /materials/{id}``, a SECOND soft-delete writer of
``parts.is_active`` that does not record the sidecar. ``restore_part`` reads NULL as "prior
state unknown" and, by OWNER DECISION, restores those rows INACTIVE:
COALESCE(is_active_before_delete, FALSE).

Tightening the column to NOT NULL (with either default) would have to invent a prior
``is_active`` for already-deleted parts -- fabricating catalog state in a quality record,
since a part number is a controlled identifier on drawings, BOM lines and receiving
transactions -- and would make the fallback branch unreachable. There is deliberately no
...
REVISION_086 = "086_part_active_before_delete"
MIGRATION_FILE = "086_part_active_before_delete.py"
...
def _load_module():
    ...
    spec = importlib.util.spec_from_file_location("_migtest_086", path)
...
def test_revision_id_fits_alembic_version_varchar32():
    ...
    assert len(REVISION_086) <= 32


@pytest.mark.unit
def test_module_loads_and_exposes_upgrade_downgrade():
    module = _load_module()
    assert module.revision == REVISION_086
...
def test_upgrade_adds_a_nullable_column_with_no_server_default():
    ...
    operational: NULL is the value that means "deleted before 086 shipped (or through
    ``DELETE /materials/{id}``), prior state unknown", and ``restore_part`` resolves that
    unknown to INACTIVE. A NOT NULL column with a default would have to invent catalog
    state for those rows -- and would make the unknown unrepresentable, which is the whole
    point of the branch.
    """
    body = _body()
...
def test_no_backfill_and_no_raw_dml():
    ...
    An already-deleted part's prior ``is_active`` is genuinely unknown (the delete
    overwrote it in place), so any backfill would fabricate catalog state. Being DDL-only
    also makes the migration structurally incapable of touching the tamper-evident
    audit_log hash chain (invariant 2).
    """
    body = _body()
    for forbidden in ("op.bulk_insert", "op.execute", "sa.text(", "conn.execute", "UPDATE "):
        assert forbidden not in body, f"086 must be pure DDL; found {forbidden}"


@pytest.mark.unit
def test_column_add_only_so_no_rls_needed():
    """086 creates no table, so the ENABLE ROW LEVEL SECURITY new-table convention does not
    apply (the 059-gate test keys off op.create_table). ``parts`` is an EXISTING table that
    predates 059 and already carries RLS plus the TenantMixin non-null ``company_id`` +
    index. ALTERing a table does not change its RLS state, so the Supabase Security
    Advisor's ``rls_disabled_in_public`` check cannot re-flag it. Stated as a test rather
...
def test_no_index_is_built():
    """The column is read row-wise off an already-resolved part inside ``restore_part``
...
def test_the_table_already_carries_tenant_scoping():
    """Invariant 1 is unaffected: 086 adds no new tenant surface. Asserted rather than
    ...
    company_id = Part.__table__.columns["company_id"]
...
def test_model_declares_one_nullable_unindexed_boolean_with_no_defaults():
    ...
    every NEW part row carry a remembered state it never had, which is the same fabrication
    a server default would commit at the DB level.
    """
    from app.models.part import Part

    col = Part.__table__.columns[COLUMN]
    assert isinstance(col.type, sa.Boolean)
    assert col.nullable is True, "NULL must stay reachable -- it means 'deleted before 086'"
...
@pytest.mark.unit
def test_the_sidecar_sits_beside_the_columns_it_shadows():
    """``is_active`` must still exist and still be the flag the sidecar remembers.

    The tempting "fix" this change refused was to delete ``delete_part``'s
    ``is_active = False`` write so restore has nothing to put back -- a security
    regression, because that write is a deliberate second layer behind the ``is_deleted``
    filters (``GET /parts/`` and ``GET /materials/`` both default ``active_only=True``).
    If someone later removes the ``is_active`` COLUMN instead, this sidecar becomes
    meaningless rather than merely unused -- so its continued existence is asserted here,
    next to the reason.
    """
    from app.models.part import Part

    assert "is_active" in Part.__table__.columns
    assert isinstance(Part.__table__.columns["is_active"].type, sa.Boolean)
    # ``status`` is written together with ``is_active`` by both the delete and the
    # activate/deactivate verbs ("inactive implies obsolete"), and restore resolves it
    # from whatever ``is_active`` resolves to.
    assert "status" in Part.__table__.columns
    # And the soft-delete columns the sidecar's lifetime is scoped to.
    for soft_delete_column in ("is_deleted", "deleted_at", "deleted_by"):
        assert soft_delete_column in Part.__table__.columns


@pytest.mark.unit
def test_the_sidecar_is_absent_from_every_request_schema():
    """No ``PUT`` may pre-seed the value the next restore is going to read back.

    ``PUT /parts/{id}`` and ``PUT /materials/{id}`` are blind ``setattr`` loops over
    whatever their schema accepted, so a sidecar reachable from either would stop being a
    control and become a request field. Asserted here as well as behaviourally in
    ``tests/test_part_activation.py`` because it is a property of the SCHEMAS, not of one
    endpoint's handling.
    """
    from app.schemas.part import PartBase, PartCreate, PartUpdate

    for schema in (PartBase, PartCreate, PartUpdate):
        assert COLUMN not in schema.model_fields, f"{schema.__name__} can pre-seed the restore sidecar"
...
def test_migration_086_upgrade_downgrade_upgrade_round_trip(tmp_path):
    ...
    try:
        ...
        _alembic(db_url, "upgrade", REVISION_086)
        ...
        # Batch mode REBUILDS the table on SQLite, so the neighbouring schema has to be
        # checked rather than assumed -- a rebuild that quietly lost one of these would
        # be a far worse bug than the column it was dropping.
        ...
        # The per-company unique constraint every part-number duplicate probe depends on.
        uniques = {u["name"] for u in sa.inspect(engine).get_unique_constraints(TABLE)}
        assert "uq_parts_company_part_number" in uniques
        # …and the index/CHECK migrations 079 and 080 restored after the create_all+stamp
        # bootstrap had skipped them. Losing either to a batch rebuild would silently undo
        # that repair.
        indexes = {i["name"] for i in sa.inspect(engine).get_indexes(TABLE)}
        assert "ix_parts_company_active" in indexes
        checks = {c["name"] for c in sa.inspect(engine).get_check_constraints(TABLE)}
        assert "chk_parts_standard_cost_non_negative" in checks
...
@pytest.mark.integration
@pytest.mark.slow
def test_pre_existing_parts_land_on_null_not_a_fabricated_value(tmp_path):
    """THE data claim, executed rather than asserted from the source.

    A part that exists BEFORE the column does -- including one already soft-deleted, the
    case the fallback exists for -- must come out of the upgrade with NULL. NULL is what
    ``restore_part`` reads as "prior state unknown", restoring the row INACTIVE. A server
    default or a backfill would put a fabricated prior ``is_active`` onto a controlled
    catalog identifier instead, and a source-grep cannot prove its absence the way running
    it can.
    """
    ...
    try:
        ...
        with engine.begin() as conn:
            conn.execute(sa.text("INSERT INTO companies (id, name, slug, is_active) VALUES (1, 'Werco', 'werco', 1)"))
            # Two pre-086 parts: one live-but-DEACTIVATED, one already soft-deleted.
            # The second is exactly the row the NULL fallback was written for.
            conn.execute(
                sa.text(
                    "INSERT INTO parts (part_number, name, part_type, unit_of_measure, status, "
                    "is_active, is_deleted, company_id) "
                    "VALUES ('PRE086A', 'Retired sheet', 'raw_material', 'each', 'obsolete', 0, 0, 1)"
                )
            )
            conn.execute(
                sa.text(
                    "INSERT INTO parts (part_number, name, part_type, unit_of_measure, status, "
                    "is_active, is_deleted, company_id) "
                    "VALUES ('PRE086B', 'Deleted sheet', 'raw_material', 'each', 'obsolete', 0, 1, 1)"
                )
            )
        ...
        with engine.begin() as conn:
            rows = dict(
                conn.execute(
                    sa.text(f"SELECT part_number, {COLUMN} FROM parts WHERE part_number LIKE 'PRE086%'")
                ).fetchall()
            )
        assert rows == {"PRE086A": None, "PRE086B": None}, "086 must not invent a prior is_active for existing rows"
    finally:
        engine.dispose()
