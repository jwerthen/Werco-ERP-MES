...
def _quote_config_counts(db_session, company_id: int) -> dict[str, int]:
    return {
        "materials": db_session.query(QuoteMaterial)
        .filter(QuoteMaterial.company_id == company_id)
        .count(),
        "machines": db_session.query(QuoteMachine)
        .filter(QuoteMachine.company_id == company_id)
        .count(),
        "finishes": db_session.query(QuoteFinish)
        .filter(QuoteFinish.company_id == company_id)
        .count(),
        "settings": db_session.query(QuoteSettings)
...
def test_seed_quote_config_is_tenant_scoped_and_idempotent(monkeypatch, db_session):
    ...
    assert (
        db_session.query(QuoteMaterial)
        .filter(QuoteMaterial.company_id.is_(None))
        .count()
        == 0
    )
    assert (
        db_session.query(QuoteMachine).filter(QuoteMachine.company_id.is_(None)).count()
        == 0
    )
    assert (
        db_session.query(QuoteFinish).filter(QuoteFinish.company_id.is_(None)).count()
        == 0
    )
    assert (
        db_session.query(QuoteSettings)
        .filter(QuoteSettings.company_id.is_(None))
        .count()
        == 0
    )
