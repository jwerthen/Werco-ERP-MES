...
class MRPService:
    ...
    def generate_run_number(self) -> str:
        ...
        last_run = (
            ...
            .filter(MRPRun.run_number.like(f"{prefix}%"))
            .order_by(MRPRun.run_number.desc())
            .first()
        )
    ...
    def get_work_order_requirements(self, horizon_end: date, include_allocated: bool = True) -> List[Dict]:
        ...
        for wo in work_orders:
            ...
            bom_requirements = self.explode_bom_for_mrp(
                bom.id, qty_remaining, wo.due_date or date.today() + timedelta(days=7), 0
            )
    ...
    def explode_bom_for_mrp(
        self, bom_id: int, parent_qty: float, required_date: date, level: int, visited: set = None
    ) -> List[Dict]:
    ...
    def calculate_shortages_and_actions(
        self, aggregated: Dict[int, Dict], include_safety_stock: bool = True
    ...
    def run_mrp(
        ...
        try:
            ...
            req_records, action_records = self.calculate_shortages_and_actions(aggregated, include_safety_stock)
