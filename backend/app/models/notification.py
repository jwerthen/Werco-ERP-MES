...
class NotificationLog(Base, TenantMixin):
    ...
    # Email jobs also use these fields for durable queued/accepted/failure states
    # and an opaque Message-ID. Legacy email rows remain NULL (unverified).
    ...
    # Email status filtering is bounded by tenant/user/date. A future Twilio
    # status-callback webhook looking rows up by SID must add an index on
