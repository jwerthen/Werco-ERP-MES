...
from app.middleware.fabrication_quote_body_limit import FabricationQuoteBodyLimitMiddleware
...
async def lifespan(app: FastAPI):
    ...
    #
    # Skipped under ENVIRONMENT=test for a
...
tags_metadata = [
    ...
    {
        "name": "Fabrication Quoting",
        "description": "Reviewed fabrication estimates, source evidence and versioned process profiles",
    },
    {
        "name": "OEE Tracking",
...
app.add_middleware(FabricationQuoteBodyLimitMiddleware)
...
def _json_body_limit(request: Request) -> int:
    prefix = settings.API_V1_PREFIX.rstrip('/') + '/fabrication-quotes'
...
def _json_body_too_large_response(request: Request, size: int) -> JSONResponse:
    ...
    """
    limit = _json_body_limit(request)
...
async def limit_json_body_size(request: Request, call_next):
    ...
    if request.method in ("POST", "PUT", "PATCH") and request.headers.get("content-type", "").startswith(
        ...
        max_body_bytes = _json_body_limit(request)
