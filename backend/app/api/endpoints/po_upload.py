...
async def _upload_and_extract_document(
    ...
    if file_ext not in SUPPORTED_EXTENSIONS:
        raise HTTPException(status_code=400, detail="Unsupported file type. Allowed: PDF, DOC, DOCX")
    ...
    try:
        ...
        logger.info(
            f"[PO_UPLOAD] Extraction result - file_type: {extraction_result.file_type}, confidence: {extraction_result.confidence}, text_length: {len(extraction_result.text)}"
        )
        ...
        if not extraction_result.text or len(extraction_result.text.strip()) < 50:
            ...
            return POExtractionResult(
                ...
                pdf_path=doc_path,
                validation_issues=[{"field": "document_content", "severity": "error", "message": error_msg}],
            )
        ...
        if "_error" in llm_result:
            return POExtractionResult(
                ...
                pdf_page_count=extraction_result.page_count,
                pdf_path=doc_path,
                validation_issues=[{"field": "extraction", "severity": "error", "message": llm_result["_error"]}],
            )
        ...
        result = POExtractionResult(
            ...
            po_number=po_number,
            quote_number=llm_result.get("quote_number"),
            vendor=VendorExtracted(name=vendor_data.get("name"), address=vendor_data.get("address")),
