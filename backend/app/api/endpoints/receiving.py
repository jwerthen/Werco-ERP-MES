...
from app.models.quality import NCRDisposition, NCRSource, NCRStatus, NonConformanceReport
...
def generate_receipt_number(db: Session) -> str:
    ...
    last = (
        db.query(POReceipt)
        .filter(POReceipt.receipt_number.like(f"{prefix}%"))
...
def log_audit(db: Session, user: Optional[User], action: str, resource_type: str, resource_id: int, details: str):
...
def get_inspection_queue(
    ...
    receipts = (
        ...
        .filter(POReceipt.status == ReceiptStatus.PENDING_INSPECTION, POReceipt.received_at >= cutoff)
...
def inspect_receipt(
    ...
    result = InspectionResultResponse(
        receipt=ReceiptResponse.model_validate(receipt), inventory_created=False, ncr_created=False
    )
    ...
    if inspection.quantity_rejected > 0:
        ncr = _create_ncr_for_rejection(
            db, receipt, inspection, current_user, po.vendor.name if po.vendor else None, po.po_number, part
        )
...
def get_receiving_stats(
    ...
    pending_count = (
        db.query(func.count(POReceipt.id)).filter(POReceipt.status == ReceiptStatus.PENDING_INSPECTION).scalar()
    )

    # Count received in period
    received_count = db.query(func.count(POReceipt.id)).filter(POReceipt.received_at >= cutoff).scalar()

    # Count inspected in period
    inspected_count = (
        db.query(func.count(POReceipt.id))
        .filter(POReceipt.inspected_at >= cutoff, POReceipt.status != ReceiptStatus.PENDING_INSPECTION)
        .scalar()
    )
    ...
    rejection_count = (
        db.query(func.count(POReceipt.id))
        .filter(POReceipt.inspected_at >= cutoff, POReceipt.quantity_rejected > 0)
        .scalar()
    )
...
def get_receiving_locations(
    ...
    locations = (
        ...
        .filter(InventoryLocation.is_active == True, InventoryLocation.is_receivable == True)
