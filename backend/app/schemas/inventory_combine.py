...
class PartStockSummarySchema(UTCModel):
    ...
    ``eligible_available`` is the same figure across the ELIGIBLE rows only, and
    it is the number that actually bounds the move -- the two differ by exactly
    the stock that is on hold, quarantined, rejected or on a deactivated row.
    """
...
class InventoryCombineRequest(BaseModel):
    ...
    ``quantity`` is ``gt=0``, so a literal ``0`` is a **422** before the handler
    runs -- distinct from the **409** ``no_available_stock`` a second combine of an
    already-drained SKU gets.
    """

    source_part_id: int
    target_part_id: int
    quantity: float = Field(gt=0, description="How much to move from the source SKU onto the target SKU.")
    reason: str = Field(
        ...,
        min_length=5,
        max_length=500,
