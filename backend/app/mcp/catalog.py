...
def resolve_schema(
    ...
    *,
    ...
    for key, value in schema.items():
        if key == "title":
            # Pydantic's auto-titles ("Work Order Id") are noise next to the property name.
            continue
...
def build_catalog(
    ...
    *,
    ...
    Names are assigned over EVERY secured operation BEFORE the shadowed pairs are
    dropped, so shadowing never renames a neighbour: the Routing router's
    ...
    """
    shadowed_set: Set[Tuple[str, str]] = set(shadowed)
    ...
    tools = [
        ...
        if (method, path) not in shadowed_set
    ]
