...
def build_plan(rows, db, company_id: int):
    ...
    # Existing lookups
    existing_customers = {c.name: c for c in db.query(Customer).filter(Customer.company_id == company_id).all()}
    existing_parts = {
        (p.part_number, (p.revision or "")): p for p in db.query(Part).filter(Part.company_id == company_id).all()
    }
    existing_wo_numbers = {
        wo for (wo,) in db.query(WorkOrder.work_order_number).filter(WorkOrder.company_id == company_id).all()
    }
    ...
    for r in rows:
        ...
        if not pn:
            ...
            r["part_number"] = pn
            r["rev"] = rev
            warnings.append(f"Row {r['_row']} (WO {r['wo_number']}): blank part #, " f"creating placeholder {pn!r}")
...
def commit_plan(db, company_id, created_by, customers, parts, wos):
    customer_map = {c.name: c for c in db.query(Customer).filter(Customer.company_id == company_id).all()}
...
def main():
    ...
    parser.add_argument("--company-id", type=int, required=True, help="Tenant company_id to import under")
    parser.add_argument("--created-by", type=int, default=None, help="User id to stamp on created_by / released_by")
    ...
    try:
        ...
        print_plan(customers, parts, wos, warnings, open_count=len(open_rows), total_count=len(all_rows))
        ...
        db.commit()
        print(f"Done. Created {len(customers)} customers, {len(parts)} parts, " f"{len(wos)} work orders.")
    except Exception:
        db.rollback()
        raise
    finally:
