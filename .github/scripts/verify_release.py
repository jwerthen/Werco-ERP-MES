...
def main() -> int:
    ...
    print(f"Waiting for {args.label} to report release {expected} at {url}")
    ...
    while time.monotonic() < deadline:
        ...
        try:
            ...
            if seen == expected:
                print(f"Attempt {attempt}: {args.label} is serving {expected}.")
                return 0
            ...
            print(f"Attempt {attempt}: {args.label} still reports {last!r}")
        except (urllib.error.URLError, OSError) as exc:
            # Expected while the container restarts behind the load balancer.
            last = f"<unreachable: {exc}>"
            print(f"Attempt {attempt}: {args.label} {last}")
    ...
    print(
        ...
        f"Last seen: {last}\n"
        "The upload reached Railway (the step before this one proved a build was created), "
        "so the build or the container start most likely failed -- open the Build Logs URL "
        "printed by the deploy step above.",
        file=sys.stderr,
    )
    return 1
